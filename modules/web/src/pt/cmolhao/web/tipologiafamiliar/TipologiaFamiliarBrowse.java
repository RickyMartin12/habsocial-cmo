...
public class TipologiaFamiliarBrowse extends StandardLookup<TipologiaFamiliar> {
    ...
    @Named("tipologiaFamiliarsTable.remove")
    protected RemoveAction<TipologiaFamiliar> tipologiaFamiliarsTableRemove;
    @Inject
    ...
    @Subscribe("tipologiaFamiliarsTable.remove")
    protected void onTipologiaFamiliarsTableRemove(Action.ActionPerformedEvent event) {
        tipologiaFamiliarsTableRemove.setConfirmation(false);
        if (tipologiaFamiliarsTable.getSelected().isEmpty())
        {
            dialogs.createOptionDialog()
                    .withCaption("Selecção da tipologia familiar")
                    .withMessage("Deve seleccionar pelo umas das tipologias familiares")
                    .withActions(
        ...
        {
            TipologiaFamiliar user = tipologiaFamiliarsTable.getSingleSelected();
            dialogs.createOptionDialog()
                    .withCaption("Remover a linha da tabela da tipologia familiar número '"+user.getId()+"' ")
                    .withMessage("Tens a certeza que quer remover esta linha da tabela da tipologia familiar número '"+user.getId()+"'?")
                    .withActions(
                            new DialogAction(DialogAction.Type.YES)
                                    .withHandler(e ->
                                    {
                                        tipologiaFamiliarsTableRemove.execute();
                                    }),
