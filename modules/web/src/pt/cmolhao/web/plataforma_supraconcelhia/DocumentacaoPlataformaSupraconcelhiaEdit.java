...
public class DocumentacaoPlataformaSupraconcelhiaEdit extends StandardEditor<DocumentacaoPlataformaSupraconcelhia> {
    @Inject
    protected InstanceContainer<DocumentacaoPlataformaSupraconcelhia> documentacaoPlataformaSupraconcelhiaDc;
    @Inject
    ...
    protected void onInit(InitEvent event) {
        ...
        documentacaoPlataformaSupraconcelhiaDc.addItemPropertyChangeListener(employeeItemPropertyChangeEvent -> {
    ...
    protected void onAfterShow(AfterShowEvent event) {
        ...
        getWindow().setCaption("Adicionar/Editar Documentacao da Plataforma de Supraconcelhia - " + idDocumentacao.getValue());
        ...
        for (RedeTrabalho item : customers) {
            if (item.getNome().equals("Plataforma Supraconcelhia"))
            {
    ...
    protected void onIdRedeTrabalhoFieldValueChange(HasValue.ValueChangeEvent<RedeTrabalho> event) {
        ...
        for (SubRedeTrabalho item : customers) {
            if (item.getIdRedeTrabalho().getId().equals(201))
            {
                if (item.getId() != null)
                {
                    map.put(item.getNome(), item);
