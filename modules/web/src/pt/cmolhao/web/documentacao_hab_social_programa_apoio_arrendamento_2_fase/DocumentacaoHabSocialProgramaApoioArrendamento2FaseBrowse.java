...
import pt.cmolhao.web.documentacao_hab_social_programa_apoio_arrendamento_1_fase.DocumentacaoHabSocialProgramaApoioArrendamento1FaseEdit;
...
public class DocumentacaoHabSocialProgramaApoioArrendamento2FaseBrowse extends StandardLookup<DocumentacaoHabSocialProgramaApoioArrendamento2Fase> {
    @Inject
    protected CollectionLoader<DocumentacaoHabSocialProgramaApoioArrendamento2Fase> documentacaoHabSocialProgramaApoioArrendamento2FasesDl;
    @Inject
    ...
    @Inject
    protected GroupTable<DocumentacaoHabSocialProgramaApoioArrendamento2Fase> documentacaoHabSocialProgramaApoioArrendamento2FasesTable;
    @Named("documentacaoHabSocialProgramaApoioArrendamento2FasesTable.remove")
    protected RemoveAction<DocumentacaoHabSocialProgramaApoioArrendamento2Fase> documentacaoHabSocialProgramaApoioArrendamento2FasesTableRemove;
    ...
    protected void onAfterShow(AfterShowEvent event) {
        ...
        if(idRedeTrabalhoField.getValue() != null)
        {
            ...
            for (SubRedeTrabalho item2 : customers2) {
                ...
                {
                    if (item2.getNome().equals("2ª fase"))
                    {
    ...
    protected void onInit(InitEvent event) {
        ...
        documentacaoHabSocialProgramaApoioArrendamento2FasesTable.setEmptyStateLinkClickHandler(emptyStateClickEvent ->
                screenBuilders.editor(documentacaoHabSocialProgramaApoioArrendamento2FasesTable)
                        .newEntity()
                        ...
                        .withScreenClass(DocumentacaoHabSocialProgramaApoioArrendamento2FaseEdit.class)    // specific editor screen
                        .build()
                        .show()
        );
        ...
        documentacaoHabSocialProgramaApoioArrendamento2FasesTable.addGeneratedColumn("file", entity -> {
    ...
    protected void onReset_documentacaoClick(Button.ClickEvent event) {
        ...
        documentacaoHabSocialProgramaApoioArrendamento2FasesDl.removeParameter("idHabitacaoSocial");
        documentacaoHabSocialProgramaApoioArrendamento2FasesDl.removeParameter("numDocumento");
        documentacaoHabSocialProgramaApoioArrendamento2FasesDl.load();
    }
    ...
    protected void onSearch_documentacaoClick(Button.ClickEvent event) {
        if (habitacaoSocialField.getValue() != null) {
            documentacaoHabSocialProgramaApoioArrendamento2FasesDl.setParameter("idHabitacaoSocial",  habitacaoSocialField.getValue().getId());
        } else {
            documentacaoHabSocialProgramaApoioArrendamento2FasesDl.removeParameter("idHabitacaoSocial");
        }
        ...
        if (idRedeTrabalhoField.getValue() != null) {
            documentacaoHabSocialProgramaApoioArrendamento2FasesDl.setParameter("idRedeTrabalho",  idRedeTrabalhoField.getValue().getId());
        } else {
            documentacaoHabSocialProgramaApoioArrendamento2FasesDl.removeParameter("idRedeTrabalho");
        }

        if (idSubRedeTrabalhoField.getValue() != null) {
            documentacaoHabSocialProgramaApoioArrendamento2FasesDl.setParameter("idSubRedeTrabalho",  idSubRedeTrabalhoField.getValue().getId());
        } else {
            documentacaoHabSocialProgramaApoioArrendamento2FasesDl.removeParameter("idSubRedeTrabalho");
        }
        ...
        if (numeroDocumentacaoField.getValue() != null) {
            if (isNumeric(numeroDocumentacaoField.getValue()))
            {
                documentacaoHabSocialProgramaApoioArrendamento2FasesDl.setParameter("numDocumento", "(?i)%" + numeroDocumentacaoField.getValue() + "%" );
            }
            else
            {
                notifications.create()
    ...
    protected void onLinhasDocumentacaoValueChange(HasValue.ValueChangeEvent event) {
        ...
        {
            documentacaoHabSocialProgramaApoioArrendamento2FasesDl.setMaxResults(Integer.parseInt(event.getValue().toString()));
        }
        else
        {
            documentacaoHabSocialProgramaApoioArrendamento2FasesDl.setMaxResults(0);
        }
        documentacaoHabSocialProgramaApoioArrendamento2FasesDl.load();
    }

    @Subscribe("documentacaoHabSocialProgramaApoioArrendamento2FasesTable.remove")
    protected void onDocumentacaoHabSocialProgramaApoioArrendamento2FasesTableRemove(Action.ActionPerformedEvent event) {
        documentacaoHabSocialProgramaApoioArrendamento2FasesTableRemove.setConfirmation(false);
        if (documentacaoHabSocialProgramaApoioArrendamento2FasesTable.getSelected().isEmpty())
        {
            dialogs.createOptionDialog()
                    .withCaption("Selecção de documentos de habitação social no Programa de Apoio ao Arrendamento na 2ª fase")
                    .withMessage("Deve seleccionar pelo um dos documentos de habitação social no Programa de Apoio ao Arrendamento na 2ª fase")
                    .withActions(
        ...
        {
            DocumentacaoHabSocialProgramaApoioArrendamento2Fase user = documentacaoHabSocialProgramaApoioArrendamento2FasesTable.getSingleSelected();
            dialogs.createOptionDialog()
                    .withCaption("Remover a linha da tabela do documentação de habitação social no Programa de Apoio ao Arrendamento na 2ª fase número '"+user.getId()+"' ")
                    .withMessage("Tens a certeza que quer remover esta linha da tabela do documentação de habitação social no Programa de Apoio ao Arrendamento na 2ª fase número '"+user.getId()+"'?")
                    .withActions(
                            new DialogAction(DialogAction.Type.YES)
                                    .withHandler(e ->
                                    {
                                        documentacaoHabSocialProgramaApoioArrendamento2FasesTableRemove.execute();
                                    }),
