...
public class DocumentacaoGruposTrabalhoEdit extends StandardEditor<DocumentacaoGruposTrabalho> {
    @Inject
    protected InstanceContainer<DocumentacaoGruposTrabalho> documentacaoGruposTrabalhoDc;
    @Inject
    ...
    protected void onInit(InitEvent event) {
        ...
        documentacaoGruposTrabalhoDc.addItemPropertyChangeListener(employeeItemPropertyChangeEvent -> {
    ...
    protected void onAfterShow(AfterShowEvent event) {
        ...
        getWindow().setCaption("Adicionar/Editar Documentacao de Grupos de Trabalho - " + idDocumentacao.getValue());
        ...
        for (RedeTrabalho item : customers) {
            if (item.getNome().equals("Grupos de Trabalho"))
            {
    ...
    protected void onIdRedeTrabalhoFieldValueChange(HasValue.ValueChangeEvent<RedeTrabalho> event) {
        ...
        for (SubRedeTrabalho item : customers) {
            if (item.getIdRedeTrabalho().getId().equals(3))
            {
                if (item.getId() != null)
                {
                    map.put(item.getNome(), item);
